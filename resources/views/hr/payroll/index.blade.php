    ...
    <div class="p-6 mb-6 bg-white rounded shadow-md">
        <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex mb-4 space-x-4">
                <a href="{{ route('hr.dashboard') }}" class="inline-block p-2 px-4 text-white bg-gray-600 rounded hover:bg-gray-700">
        ...
        <form action="{{ route('hr.payroll.index') }}" method="GET" class="flex items-center space-x-4">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($payrolls as $payroll)
                        <tr>
                            ...
                            <td class="px-4 py-2">{{ number_format($payroll->basic_salary, 2) }}</td>
                            <td class="px-4 py-2">{{ number_format($payroll->allowances, 2) }}</td>
                            <td class="px-4 py-2">{{ number_format($payroll->deductions, 2) }}</td>
                            <td class="px-4 py-2">{{ $payroll->overtime_hours }}</td>
                            <td class="px-4 py-2">{{ number_format($payroll->overtime_rate, 2) }}</td>
                            <td class="px-4 py-2">{{ number_format($payroll->overtime_pay, 2) }}</td>
                            <td class="px-4 py-2">{{ number_format($payroll->bonus, 2) }}</td>
                            <td class="px-4 py-2">{{ number_format($payroll->net_salary, 2) }}</td>
